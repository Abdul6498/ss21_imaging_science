...

{
    ...
    */
    ...
    float *help_y_1;
    float *help_y_2;
    alloc_vector (&help_x, nx+length+length);
    ...
    {
        // copy in row vector
        for (p=length; p<nx+length; p++)
        {
            help_x[p] = u[j][p-length];
        }

        // reflecting at the boundary
        for (p=1; p<length; p++)
        {
            help_x[length-p]      = help_x[length+p-1];
            help_x[nx+length-1+p] = help_x[nx+length-p];
        }

        // convolution along row (x-direction)
        float sum_dx = 0;
        float sum_by = 0;
        for (p=length; p<=nx+length-1; p++)
        {
            for(int k=0; k<=length; k++)
            {
                sum_dx += help_x[p+k]*deri[k];
                sum_by += help_x[p+k]*bino[k];
            }
            dx[j][p-length+1] = sum_dx;
            dy[j][p-length+1] = sum_by;            
        }
    }
    disalloc_vector (help_x, nx+length+length);

    alloc_vector (&help_y_1, ny+length+length);
    alloc_vector (&help_y_2, ny+length+length);
    for (i=1; i<=nx; i++)
    {
        // copy column vector
        for (p=length; p<ny+length; p++)
        {
            help_y_1[p] = dx[p-length][i];
            help_y_2[p] = dy[p-length][i];
        }

        // reflecting at the boundary
        for (p=1; p<length; p++)
        {
            help_y_1[length-p]      = help_y_1[length+p-1];
            help_y_1[ny+length-1+p] = help_y_1[ny+length-p];

            help_y_2[length-p]      = help_y_2[length+p-1];
            help_y_2[ny+length-1+p] = help_y_2[ny+length-p];                
        }

        // convolution along column (y-direction)
        float sum_bx = 0;
        float sum_dy = 0;
        for (p=length; p<=ny+length-1; p++)
        {
            for(int k=0; k<=length; k++)
            {
                sum_bx += help_y_1[p+k]*bino[k];
                sum_dy += help_y_2[p+k]*deri[k];
            }
            dx[p-length+1][i] = sum_bx;
            dy[p-length+1][i] = sum_dy;
        }
    }
    disalloc_vector (help_y_1, ny+length+length);
    disalloc_vector (help_y_2, ny+length+length);
}
